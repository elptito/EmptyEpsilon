...
P<SpaceObject> GuiDockingButton::findDockingTarget()
{
    ...
    {
        dock_object = obj;
        if (dock_object && dock_object->canBeDockedBy(target_spaceship) && (dock_object->getPosition() - target_spaceship->getPosition()) < 1000.0f + dock_object->getRadius())
            break;
        dock_object = NULL;
    }
    return dock_object;
