...
: GuiElement(owner, "COMMS_OVERLAY")
{
    ...
    (new GuiLabel(closed_box, "COMMS_BROKEN_LABEL", "Communication coupee", 40))->setSize(GuiElement::GuiSizeMax, GuiElement::GuiSizeMax)->setPosition(0, 0, ATopLeft);
    ...
    script_comms_close = new GuiButton(script_comms_box, "CLOSE_BUTTON", "Fin", [this]() {
        script_comms_options->setOptions({});
        if (my_spaceship)
            my_spaceship->commandCloseTextComm();
    });
...
void GuiCommsOverlay::onDraw(sf::RenderTarget& window)
{
    ...
    if (my_spaceship)
    {
        ...
        hailed_label->setText("Contacte par " + my_spaceship->getCommsTargetName());
