...
: GuiElement(owner, id)
{
    button = new GuiToggleButton(this, id + "_BUTTON", "Bouclier: ON", [](bool value) {
        if (my_spaceship)
    ...
    if (my_spaceship)
    {
        ...
        else
        {
            ...
            if (gameGlobalInfo->use_beam_shield_frequencies)
                button->setText(frequencyToString(my_spaceship->shield_frequency) + (my_spaceship->shields_active ? " Bouclier: ON" : " Bouclier: OFF"));
            else
                button->setText(my_spaceship->shields_active ? " Bouclier: ON" : " Bouclier: OFF");
        }
    }
}
