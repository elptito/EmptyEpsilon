...
: GuiOverlay(owner, id, sf::Color(0,0,0,64))
{
    ...
    close_button = new GuiButton(minigame_box, "", "Fin", [this]()
    {
        hide();
    });
...
void GuiHackingDialog::onDraw(sf::RenderTarget& window)
{
    ...
    if (game->isGameComplete())
    {
        ...
        status_label->setText("Piratage en cours: " + string(int(100 * game->getProgress())) + "%");
    }
    if (target_system != "")
    {
        std::vector<std::pair<string, float> > targets = target->getHackingTargets();
        for(std::pair<string, float>& target : targets)
        {
            ...
            {
                hacking_status_label->setText("Piratage " + target_system + ": " + string(int(target.second * 100.0f + 0.5f)) + "%");
                break;
            }
        }
    }
...
void GuiHackingDialog::onMiniGameComplete(bool success)
{
    ...
    status_label->setText(success ? "Piratage : SUCCESS!" : "Piratage : ECHEC!");
}
