...
bool HardwareController::getVariableValue(const ShipFilter& ship_filter, string variable_name, float& value)
{
    ...
    for(int n=0; n<max_beam_weapons; n++){
        SHIP_VARIABLE("BeamCharging" + string(n), ship->beam_weapons[n].getCooldown() > 0.0f ? 1.0f : 0.0f);
    }
    for(int n=0; n<max_weapon_tubes; n++)
    {
        ...
        SHIP_VARIABLE("TubeFired" + string(n), ship->weapon_tube[n].getMissilesFired());
    }
